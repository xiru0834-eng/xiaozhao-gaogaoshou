...
状态：阶段 1 数据基础 T01–T05 已实现并验收；T07/T08 的模型子集工程验证通过，真实提供者未验收；T06 原生助手仍待实现。2026-09-20 已完成 [本轮复测](../docs/阶段1和2-复测记录-20260920.md)，新增 [阶段 3 需求](../docs/阶段3-手动岗位采集需求.md) 和 [阶段 3 细化计划](../docs/阶段3-实施计划.md)，尚未实现采集。前置 TS 迁移详见 [迁移记录](../docs/TypeScript迁移.md)。每个任务先补失败测试/固定输入，再实现；任务结束记录实际证据，不凭描述勾选。
...
## 阶段 C 细化切片（第三阶段，待审阅后执行）

对应 T09–T12，不是 T03；全部验收要求见 [阶段 3 实施计划](../docs/阶段3-实施计划.md)。第一步 C0，其次 C1，不先写每日定时器。

- [ ] C0：官方来源核验、60 例金标 / 40 开发 + 20 冻结划分、类型契约。
- [ ] C1：安全只读抓取、请求/体积/重定向/DNS边界、真实来源可读证据。
- [ ] C2：最小更新中心与原文查看；无模型可用、不写正式岗位。
- [ ] C3：本地最小求职条件与版本冲突；不含个人联系方式。
- [ ] C4a：正文解析、结构化字段与确定性匹配规则。
- [ ] C4b：复用模型安全边界的内部提取；证据校验和调用预算。
- [ ] C5：目录 v2 迁移前备份、运行/候选/证据持久化、异常恢复。
- [ ] C6：运行/取消/预览 API 与 UI；全失败和零新增严格区分。
- [ ] C7：同库事务接纳、幂等回执、冲突拒绝、个人进度零误改。
- [ ] C8：用户接纳及岗位展示；重开/备份/原台账回归。
- [ ] C9：固定集指标、两轮真实模型、三家真实官方来源试点；缺真实接口或 T06 不关闭完整 T12。

检查点：A 原文读取可用 → B 候选有证据 → C 预览可恢复 → D 真实闭环验收。规划中的测试集、接口、脚本均未建成，不执行不存在的命令、不宣称已通过。
