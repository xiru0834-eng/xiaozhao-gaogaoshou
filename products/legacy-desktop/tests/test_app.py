...
class PersistenceTest(unittest.TestCase):
    def test_post_accepts_no_suitable_role_status(self):
        ...
        try:
            with tempfile.TemporaryDirectory() as folder:
                ...
                self.assertIn('id="board"', page.read().decode('utf-8'))
    ...
    def test_reopen(self):
        with tempfile.TemporaryDirectory() as folder, patch.object(app, 'ROOT', pathlib.Path(folder)), patch.object(app, 'DB', pathlib.Path(folder) / 'qiuzhao.db'):
            app.initialize()
            with app.connect() as db:
