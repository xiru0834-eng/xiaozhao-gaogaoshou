 ...
 */
export function CoachPreparation({ targetRole = '', busy, onStart, kind }) {
  ...
  return h('section', { className: 'sz-preparation sz-preparation-page' }, h('h2', null, t(kind === 'mock' ? 'mockPage' : 'targetedPage')),
    ...
    kind === 'targeted' ? h(Button, { tone: 'primary', disabled: busy || !valid, onClick: () => onStart({ kind: 'targeted', preparation, targetRole: role }) }, t('targetedStart')) : null,
    kind === 'mock' ? h(React.Fragment, null, h('div', { className: 'sz-settings-row' },
    ...
    h(Button, { tone: 'primary', disabled: busy || !valid || !project.trim(), onClick: () => onStart({ kind: 'mock', preparation,
      targetRole: role, durationMinutes: duration, questionLimit: limit, difficulty, interviewerStyle: style }) }, t('mockStart'))) : null)
}
