...
export async function openCareerRepository(directory = join(defaultDataDirectory(), 'career')) {
  const { CatalogStore, Store, parseStatuses } = await import('../../lib/career-data.js')
  ...
  const catalog = new CatalogStore(join(directory, 'catalog.db'))
  let store
  try { store = new Store(join(directory, 'qiuzhao.db')) } catch (error) { catalog.close(); throw error }
  const company = (name) => {
  ...
  return { profileId, directory, catalog, store, company,
    save(updates) {
      let checked
    ...
    close() { try { store.close() } finally { catalog.close() } },
  }
}
