import { transcribe } from '../../infrastructure/speech-provider.js'
 ...
 */
export function registerSpeechRoutes(ctx, config) {
  ...
  ctx.effect(() => ctx.webServer.register({ kind: 'exact', path: '/interview/api/voice', handler: async (request, response) => {
    ...
    if (request.method === 'GET') return send(response, 200, {
      ...
      maxSeconds: config.maxSeconds, maxBytes: config.maxBytes,
    })
    ...
    try {
      ...
      const text = await transcribe(config, Buffer.concat(chunks), mime, { signal: controller.signal })
      send(response, 200, { text })
    } catch (error) {
      send(response, 502, { error: { message: error instanceof TypeError ? '语音请求无效，请检查配置后重试' : '语音转写未完成，请检查服务连接后重试；你可以继续文字作答' } })
