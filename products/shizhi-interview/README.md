...
The coach navigation opens **练习首页** and six task pages: **题库练习**, **模拟面试**, **岗位专项**, **复习计划**, **练习记录**, and **已斩题**. The current page keeps its dark teal background and white text, including on hover. The bank supports keyword search and topic filters; **练习记录** opens an ongoing practice or a completed report. Internal page switches retain the current answer draft and bring the destination heading and selected navigation into view.

The coach uses a light workspace with white content surfaces, slate text and teal controls. Its practice home displays saved bank counts, opens each subject with the matching filter, and links to mock interviews, targeted preparation and review. Existing practices restore directly to their question. The overview uses native interface content and requires no photographic assets. Mock interview settings use keyboard-accessible radio groups and a native select. An incomplete preparation form identifies and focuses the first missing field. Feedback presents the next improvement before individual findings. Controls support visible focus, disabled states and reduced motion. The workbench and coach share horizontal task navigation, content widths, typography, cards and form controls. The embedded workbench uses compact character accents instead of a large scene and duplicate branding. Its **换肤** and light/dark controls also update the coach and product header; the existing browser preference persists across reloads. Unconfirmed wardrobe previews do not change either workspace. The standalone career application retains its own layout.
