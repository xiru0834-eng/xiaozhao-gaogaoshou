...
The agent application bank contains 24 original oral questions for campus and junior roles, grouped into agent fundamentals, tools/MCP, RAG, context/security, workflow reliability, and evaluation/project discussion. Choose any pending question in **题库练习**. During ordinary practice, **本方向选题** switches between questions in the current subject and retains existing answers; **下一题** skips already practiced and explicitly mastered questions. Reviews use fixed reference criteria with links to official reading. These are not employer questions, and the cues do not prescribe a single wording for a correct answer. Maintain the content in [the agent catalog](src/domain/agent-catalog.js).
