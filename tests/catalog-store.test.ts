...
const company = (name: string, aliases: string[] = []): NewCompany => ({
  row: [
    name,
    "ai",
    "Agent",
...
test("catalog seed preserves every field/order/date and initialization is idempotent after restart", async () => {
  const root = await mkdtemp(join(tmpdir(), "xiaozhao-catalog-"));
...
test("append keeps history; normalized aliases deduplicate; conflicts and stale revisions do not mutate", async () => {
  const root = await mkdtemp(join(tmpdir(), "xiaozhao-alias-"));
  const store = new CatalogStore(join(root, "catalog.db"));
  try {
    const before = store.snapshot();
    const added = store.append(
      company("测试公司", ["Test Co"]),
      before.revision,
    );
    ...
    assert.throws(
      () => store.append(company("另一家"), before.revision),
      /revision/,
    );
    assert.throws(
      () => store.append(company("冲突", ["Test Co", "腾讯"]), after.revision),
      /alias/,
    );
    const invalid = company("无效日期");
    invalid.firstSeenDate = "2026-02-30";
    assert.throws(() => store.append(invalid, after.revision));
    const url = company("危险链接");
    url.row[6] = "javascript:alert(1)";
    assert.throws(() => store.append(url, after.revision));
    assert.deepEqual(store.snapshot(), after);
...
test("unknown catalog version is refused without rewriting the file", async () => {
  const root = await mkdtemp(join(tmpdir(), "xiaozhao-version-"));
  const path = join(root, "catalog.db");
  try {
    ...
    assert.throws(() => new CatalogStore(path), /version/);
...
test("alias insert failure rolls back company and revision together", async () => {
  const root = await mkdtemp(join(tmpdir(), "xiaozhao-catalog-failure-"));
  const path = join(root, "catalog.db");
  ...
  try {
    ...
    const db = new DatabaseSync(path);
    db.exec(
      "CREATE TRIGGER reject_alias BEFORE INSERT ON aliases WHEN NEW.key='reject-me' BEGIN SELECT RAISE(ABORT,'alias write failed'); END",
    );
    db.close();
    assert.throws(
      () => store.append(company("模拟失败", ["reject-me"]), before.revision),
      /alias write failed/,
    );
    ...
    const added = store.append(
      company(
        "合法二十别名",
        Array.from({ length: 20 }, (_, i) => "alias" + i),
      ),
      before.revision,
    );
    assert.equal(added.inserted, true);
    assert.equal(store.snapshot().metadata.at(-1)?.aliases.length, 20);
  } finally {
    store.close();
    await rm(root, { recursive: true, force: true });
  }
