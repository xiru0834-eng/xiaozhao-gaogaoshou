...
test("abort and timeout close the upstream connection without retries", async () => {
  let count = 0;
  const server = createServer((req) => { count++; req.resume(); });
  await new Promise<void>(resolve => server.listen(0, "127.0.0.1", resolve));
  const address = server.address();
  assert.ok(address && typeof address !== "string");
  ...
  try {
    await assert.rejects(generateText(config, "test", "hello", AbortSignal.timeout(100), transport), code("TIMEOUT"));
    const controller = new AbortController();
    const pending = generateText(config, "test", "hello", controller.signal, transport);
