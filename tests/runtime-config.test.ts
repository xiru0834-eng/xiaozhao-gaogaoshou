...
import {
  ...
  prepareDataDir,
  safeFile,
} from "../src/server/runtime-config.ts";
...
test("profiles have stable independent identities and exclude duplicate writers", async () => {
  ...
  try {
    const a = acquireProfile(join(root, "a"));
    const b = acquireProfile(join(root, "b"));
    assert.notEqual(a.profileId, b.profileId);
    assert.throws(() => acquireProfile(join(root, "a")), /already in use/);
    const id = a.profileId;
    a.close();
    b.close();
    const again = acquireProfile(join(root, "a"));
    assert.equal(again.profileId, id);
    again.close();
  } finally {
...
test("legacy path and junction to legacy path are rejected before writing", async () => {
  ...
  try {
    const legacy = join(root, "QiuzhaoLedger");
    await mkdir(legacy);
    await symlink(
      legacy,
      join(root, "alias"),
...
test(
  "operating system releases the profile lock after a real child process is killed",
  { timeout: 10000 },
  async (t) => {
    ...
    const child = spawn(
      ...
      [
        ...
        "--input-type=module",
        "-e",
        `import {acquireProfile} from ${JSON.stringify(moduleUrl)}; const p=acquireProfile(${JSON.stringify(root)}); console.log(p.profileId); setInterval(()=>{},1000);`,
      ],
    ...
    try {
      ...
      assert.equal(recovered.profileId, id);
      recovered.close();
    } finally {
...
test("source directory is protected regardless of launch cwd; linked database files are refused", async () => {
  const root = await mkdtemp(join(tmpdir(), "xiaozhao-file-link-"));
  const before = process.cwd();
  try {
    process.chdir(root);
    assert.throws(
      () => prepareDataDir(fileURLToPath(new URL("..", import.meta.url))),
      /source/,
    );
    await writeFile(join(root, "source.db"), "fixture");
    await link(join(root, "source.db"), join(root, "linked.db"));
    ...
    await symlink(
      process.platform === "win32"
        ? join(root, "directory-target")
        : join(root, "source.db"),
      join(root, "symbolic.db"),
