...
已新增独立 Windows 桌宠入口 `desktop_runtime.py`，可显示五角色透明动画、拖动并记住位置、右键换角色／暂停／打开工作台，双击角色打开悬浮台账。悬浮台账通过 TypeScript API 读写同一资料，不再以旧 Python 数据库为后台。

本机正式桌面入口已切换至独立桌面资料及 18766；18763 旧版与 18765 开发预览保留，不自动合并。桌宠只是桌面外壳，业务仍使用 TypeScript。未打成通用安装包，也不代表朋友拉取后会自动迁移自己的记录。

运行前必须有经过迁移验收的外部资料和 `desktop-settings.json`（包含 `dataDir`、`profileId`、`port`、`node`），缺配置会明确拒绝，不会悄悄创建空白台账。启动方式：`pythonw desktop_runtime.py --config "配置文件绝对路径"`；加 `--companion` 打开小窗，加 `--full` 打开网页。

详细范围、测试、备份和局限见 [桌面统一升级与桌宠接入](docs/桌面统一升级与桌宠接入-20260921.md)。下文的 TypeScript 独立预览和旧版兼容命令继续作为开发／回退入口，不能与已迁移的桌面资料混用。
...
顶部「岗位更新」可以选择已登记来源，先免费检查网页是否可读，再自行决定是否调用模型提取。填写并确认求职条件后，提取结果进入待核对候选；逐字段查看原文，勾选并点击「确定收录」才保存岗位，不自动改变已投状态。

- 当前仅登记速境岗位详情、MiniMax 招聘入口、Kimi 校招入口。入口页或动态空壳会如实提示限制，不代表已经覆盖全部公司。
- 模型仅接收公开正文，不自动附带简历和个人投递记录。每次最多 3 来源、3 次模型尝试；费用由所选服务决定，无自动重试。
- 运行、快照和岗位保存在本机目录库；升级 v2 前备份，接纳失败事务回滚，重复请求有回执。关闭页面不取消后台检查，进程中断不补跑。
- 工程测试和合成浏览器流程已验证；真实模型质量、三家公司真实岗位闭环、原生助手同步仍待验收。不是每日自动更新，也没有自动投递。

版本限制、独立预览和复现步骤见 [阶段 3 评测报告](docs/阶段3-评测报告.md)。测试固定合成规则用 `npm run eval:jobs`，不要将其分数解释为真实模型准确率。
