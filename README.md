...
顶部「模型设置」可配置一个自定义 **OpenAI 兼容 Chat Completions** 接口：填写公网 HTTPS Base URL、模型 ID、API Key，保存后点击「测试连接」，或输入一句话「发送并生成」。兼容的完整 `/chat/completions` 地址会自动规范化。模型名称以服务商控制台为准。
...
需求、可行性、验收标准、测试结果与限制见 [模型设置说明](docs/模型设置-需求与验收.md)。旧 Python / 桌面贴边助手尚未接入这一功能，勿在 `18763` 寻找新版模型设置。
