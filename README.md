...
| 题库练习 | 56 道原创口述题，其中 24 道面向智能体应用开发；自由选题，文字或语音回答。 |
...
截图来自真实 Electron 应用的独立演示资料，使用公共公司目录和空白个人记录，不含个人投递数据或密钥。角色插画与历史品牌海报见 [视觉素材](docs/posters/README.md)。

## 快速开始

### Windows 桌面版

使用维护者提供的 `Xiaozhao-Setup-0.1.2-x64.exe` 安装，随后打开「校招高手」快捷方式。安装包内置运行环境，使用者不需要安装 Node、Python 或 Git。安装包不存放在 Git 源码中；需要自行构建时按 [桌面版说明](products/shizhi-desktop/README.zh.md#从源码构建)操作。

首次打开可填写模型和语音密钥，也可以先跳过。桌面菜单 **校招高手 → 模型与语音配置** 用于配置 DeepSeek 和千问语音；应用右上角 **设置 → 模型** 管理聊天与点评模型。当前桌面包为未签名测试版，尚无自动更新。
...
- **备份需区分范围。** 工作台的导出菜单可下载公司目录或投递进度；完整保留练习、会话与配置，应关闭程序后备份整个数据目录，具体见 [桌面数据说明](products/shizhi-desktop/README.zh.md#数据与配置)。
- **模型会收到主动提交的内容。** AI 点评、生成、语音转写、岗位搜索或邮件分析会向已配置的服务发送相应输入；邮件账号需自行授权，每日更新需主动启用。
- **公开线索需要核实。** 初始 315 家公司来自历史快照，岗位、截止日期和推荐码以招聘官网为准。题库是原创知识练习，不是经核实的公司真题，AI 点评也可能有误。
