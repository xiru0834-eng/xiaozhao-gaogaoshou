...
Windows 用户可以使用包含运行环境的桌面安装包，首次打开直接填写模型和语音密钥。安装、数据目录和开发打包方式见 [桌面版说明](products/shizhi-desktop/README.zh.md)。源码启动需要 Node.js 24.14 或更新的 24.x、Git 和 pnpm。在仓库根目录运行：
...
> **分享版说明** · 本分支支持源码启动和 Windows 桌面安装包，桌面包需要按说明构建或从维护者获取。每个人的投递和练习记录留在自己的电脑上，当前没有在线多人同步。
