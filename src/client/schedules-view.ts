...
export function schedulesView() {
  ...
  page.innerHTML = `<div class="schedule-heading module-heading">${moduleIdentity('schedules')}<div class="module-heading-copy"><h1 id="schedule-heading" tabindex="-1">面试日程</h1><p class="schedule-subtitle">安排有条理，准备更从容。</p></div>${moduleCompanion}<div class="schedule-actions"><button class="action" data-s="back">返回台账</button><button class="action primary" data-s="new">＋ 新建日程</button></div></div>
 ...
 <section data-s="list" class="schedule-list" aria-label="全部日程" hidden></section>`;
  ...
  dialog.innerHTML = `<form data-s="form"><div class="schedule-editor-head">${moduleIdentity('schedules')}<div><small>每一场机会，都认真准备。</small><h2 id="schedule-editor-title">新建日程</h2></div><button type="button" data-s="close-editor" class="action" aria-label="关闭日程编辑">×</button></div><fieldset data-s="fields"><h3 class="schedule-form-heading">机会信息 <span>01</span></h3><div class="schedule-form-grid">
 ...
 <label>类型<select name="kind"><option value="interview">面试</option><option value="written">笔试</option></select></label><label>轮次<input name="round" maxlength="80" placeholder="例如：技术一面"></label></div><h3 class="schedule-form-heading">时间与地点 <span>02</span></h3><div class="schedule-form-grid">
 ...
 <h3 class="schedule-form-heading">准备与复盘 <span>03</span></h3><label>安排备注<textarea name="notes" rows="2" maxlength="10000" placeholder="面试要求、联系人称呼、需要携带的材料…"></textarea></label>
