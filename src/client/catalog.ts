...
export function installCatalog(value: unknown): boolean {
  ...
  if (next.revision < revision)
    throw new Error("Catalog revision moved backwards; reopen the workbench");
  if (next.revision === revision) return false;
  const nextMetadata = new Map(
    next.metadata.map((entry) => [entry.name, entry]),
  );
  DATA = next.companies;
...
export function ownershipOf(row: CompanyRow) {
  return meta(row).ownership;
}
export function recruitChannelOf(row: CompanyRow) {
  return meta(row).channel;
}
export function recruitChannelEvidence(row: CompanyRow) {
  return meta(row).channelEvidence;
}
export function aliasesOf(row: CompanyRow) {
  return meta(row).aliases;
}
export function classificationLabel(row: CompanyRow) {
  ...
  if (
    (row[F.cat] === "frn" && owner === "外企") ||
    (row[F.cat] === "soe" && owner === "央国企")
  )
    return owner;
