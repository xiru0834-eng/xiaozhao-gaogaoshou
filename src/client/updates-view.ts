...
export function updatesView() {
  ...
  page.innerHTML = `
    <button class="text-button" data-updates="back" type="button">← 返回投递工作台</button>
    <div class="updates-heading"><div><h1 id="updates-heading" tabindex="-1">岗位更新<span>手动采集</span></h1><p>先看来源与变化，再把值得保留的机会放进台账。</p></div><button class="action" data-updates="refresh" type="button">刷新记录</button></div>
    ...
    <div data-pane="start" class="updates-setup">
      <section class="updates-sources"><h2>这次检查哪些来源？</h2><p class="updates-muted">最多选 3 个。只访问已登记的公开页面，不登录、不绕过验证。</p><div data-updates="sources"></div>
      ...
      <section class="updates-preferences"><h2>你的匹配条件</h2><p class="updates-muted">条件只保存在本机，用规则判断，不交给模型猜。</p>
