...
export function readRecords(db: DatabaseSync): ApplicationRecord[] {
  ...
  const table = db
    .prepare("SELECT type FROM sqlite_master WHERE name='applications'")
    .get();
  ...
  if (
    columns.length !== 3 ||
    columns.map((c) => c.name).join(",") !== "name,status,updated_at" ||
    columns[0].pk !== 1
  )
    throw new Error("Unsupported applications schema");
  if (
    Number(db.prepare("SELECT count(*) AS n FROM applications").get()?.n) >
    100000
  )
    throw new Error("Too many progress records");
  return db
    .prepare("SELECT name,status,updated_at FROM applications ORDER BY name")
    .all()
    .map((r) => {
      if (
