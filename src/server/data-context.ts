...
export async function openDataProfile(dataDir: string) {
  ...
  try {
    ...
    for (const [name, database] of [['progress', progress], ['catalog', catalog]] as const) {
    ...
    let closed = false;
    return { profile, store, catalog: companies, backups, close() {
      if (closed) return;
      closed = true;
      try { companies.close(); } finally { try { store.close(); } finally { profile.close(); } }
    }};
  } catch (error) {
    try { catalog?.close(); } finally { try { progress?.close(); } finally { profile.close(); } }
    throw error;
  }
}
