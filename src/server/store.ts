...
export class Store {
  ...
  constructor(path: string) {
    ...
    this.db = new DatabaseSync(safeFile(dirname(path),basename(path)), { timeout: 3000 });
    try {
      ...
      if (version===0) {
        ...
        if (tables.length && !tables.some(t=>t.name==='applications')) throw new Error('Unrecognized progress database');
        if (tables.length) readRecords(this.db);
        this.db.exec('BEGIN IMMEDIATE');
        try {
          this.db.exec("CREATE TABLE IF NOT EXISTS applications (name TEXT PRIMARY KEY, status TEXT NOT NULL, updated_at TEXT NOT NULL); CREATE TABLE imports (id TEXT PRIMARY KEY,checksum TEXT NOT NULL,count INTEGER NOT NULL,imported_at TEXT NOT NULL); PRAGMA user_version=1");
          this.db.exec('COMMIT');
  ...
  records(): ApplicationRecord[] { return readRecords(this.db); }
  assertImportable(): void {
    if (this.records().length) throw new Error('Target progress is not empty');
    if (this.db.prepare('SELECT id FROM imports LIMIT 1').get()) throw new Error('Target already imported');
  }
  importRecords(records: ApplicationRecord[]): string {
    ...
    try {
      ...
      if (JSON.stringify(actual)!==JSON.stringify(records) || this.integrity()!=='ok') throw new Error('Imported records did not match snapshot');
      const checksum=createHash('sha256').update(JSON.stringify(actual)).digest('hex');
      this.db.prepare('INSERT INTO imports VALUES (?,?,?,?)').run('legacy',checksum,actual.length,new Date().toISOString());
      this.db.exec('COMMIT'); return checksum;
    } catch (error) { this.db.exec('ROLLBACK'); throw error; }
  }
  statuses(): StatusMap {
