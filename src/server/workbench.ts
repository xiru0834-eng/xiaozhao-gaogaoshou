...
import type { openDataProfile } from "./data-context.ts";
...
/** Dependencies and built assets shared by standalone and embedded workbenches. */
...
/** Owns workbench services and drains accepted requests before releasing storage.
 * @param options Built assets and provider dependencies.
 * @param context Open profile whose ownership transfers to this application.
 * @returns Authenticated request handler, background start and asynchronous disposal.
 */
export async function createWorkbench(options: WorkbenchOptions, context: Awaited<ReturnType<typeof openDataProfile>>) {
  ...
  const send = (
    ...
    res.writeHead(status, {
      ...
      "X-Frame-Options": options.embedded ? "SAMEORIGIN" : "DENY",
      "Referrer-Policy": "no-referrer",
      "Content-Security-Policy":
        `default-src 'self'; script-src 'self'; style-src 'self' 'unsafe-inline'; img-src 'self' data:; connect-src 'self'; object-src 'none'; base-uri 'none'; frame-ancestors ${options.embedded ? "'self'" : "'none'"}`,
    });
  ...
  let closed = false;
  async function handleRequest(req: IncomingMessage, res: ServerResponse, url: string) {
    if (closed) {
  ...
  return {
    handle(req: IncomingMessage, res: ServerResponse, origin: string): Promise<void> {
      const operation = handleRequest(req, res, origin);
      pending.add(operation);
      return operation.finally(() => { pending.delete(operation); });
    },
    start() { void daily.tick().catch(() => { /* Persisted reports expose scheduler failures. */ }); },
    close(): Promise<void> {
      ...
      closing = (async () => {
        ...
        try {
          ...
          try { await mail.close(); } finally { context.close(); }
        }
      })();
      return closing;
    },
