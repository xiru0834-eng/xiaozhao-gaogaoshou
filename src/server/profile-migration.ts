...
export async function importProfile(source: string, target: string) {
  if (!isAbsolute(source))
    throw new Error("Source must be an explicit absolute path");
  ...
  try {
    ...
    if (existsSync(destination)) {
      ...
      try {
        if (readRecords(existing).length)
          throw new Error("Target progress is not empty");
      } finally {
        existing.close();
      }
    }
    const original = new DatabaseSync(realSource, {
      readOnly: true,
      timeout: 3000,
    });
    const snapshot = join(
      backupDir(profile.dataDir),
      `import-${randomUUID()}.db`,
    );
    try {
    ...
    try {
      ...
      if (
        saved.prepare("PRAGMA integrity_check").get()?.integrity_check !== "ok"
      )
