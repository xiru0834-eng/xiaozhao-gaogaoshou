...
这是公开的 Windows 源码分享版，不是在线多人系统，也不是免安装 EXE。仓库提供程序和公共公司清单；每个人的投递记录保存在自己的电脑上。

当前 `main` 提供 **TypeScript 工作台 + Node.js / TypeScript 后端**。保留原界面，将目录、筛选、保存队列、接口和数据库拆成独立模块；不再把业务脚本写在 HTML 中。新增 `desktop_runtime.py` 可连接新版资料，旧 `launcher.py` 仍为独立兼容入口。桌面配置及迁移见[接入说明](桌面统一升级与桌宠接入-20260921.md)。
...
1. 访问公开仓库即可获取源码，无需将个人登录令牌写进命令或发给他人。
